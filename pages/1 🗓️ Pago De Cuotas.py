...
if st.sidebar.button("Buscar"):
    if 0 <= index_de_usuario < st.session_state.usuarios:
        if df["estado"][index_de_usuario] == "activo":
            ...
            df = pd.read_csv(st.session_state.nombre_df)
            if st.session_state.anular_usuarios and (
                    df["multas"][index_de_usuario].count("n") < 47
            ):
                Funciones.desactivar_susuario(
                    index_usuario=index_de_usuario
                )
...
if index == -1:
    ...
    st.title(
        f"№ {index} - {nombre_usuario} : {df["puestos"][index]} puesto(s)"
    )
    ...
    df_1, df_2 = Funciones.tablas_para_cuotas_y_multas(index=index)
    ...
    numero_cuotas_a_pagar = 50 - df["cuotas"][index].count("p")
    ...
    cuotas_a_pagar = st.selectbox(
        "Numero de cuotas a pagar.",
        (range(numero_cuotas_a_pagar + 1))
    )
    multas_a_pagar = st.selectbox(
